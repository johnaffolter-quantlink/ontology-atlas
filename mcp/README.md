...
The lifecycle response also includes a shadow-only `admission` receipt. It
classifies the current packet as `self_qualified`, `partial_visible_gap`,
`human_review_required`, or `hard_block`; `self_qualified` is an auto-write
candidate signal, not write permission. The existing human acceptance and
digest-bound `writePlan` gate remain authoritative until fresh field trials
prove that a later automatic-write transition is safe.

Each `constructionQualification:v1` claim also carries non-empty
`proposalRefs`. Atlas derives the expected `concept:`, `relation:`,
`competency:`, and `impact:` refs from the exact in-memory `reviewPlan` and
returns a `proposalCoverage` receipt. Missing or foreign refs, or refs whose
claims are absent from the passed source-hidden task, fail closed; this binds
...
In read-only mode the server advertises **only the 24 read tools** in
...
`pnpm test:dogfood:script-refs` checks help text and package script body `pnpm ...` references against root package scripts, source-checkout dependency availability and exact-pin parity, plus focused filter parsing and wrapper summaries.
...
Literal `.` is the explicit bound repository root. It is valid in node paths and
persisted competency Evidence/Paths, but it does not identify a canonical child
file or prove dependency/runtime completeness. Unsafe sibling paths still make
the evidence row fail closed.
...
may describe one logical relation, and the numeric deduplicated app-map census is
explicitly unavailable to MCP because the server does not own the loaded UI state.

`agent_brief` is project-scoped end to end. A multi-project vault must pass an
...
Business capability proposals are fail-closed: bounded outcome prose must
cross-check an implementation witness before a capability candidate is
emitted. FSD/source/workspace folders that have no such business clue remain
implementation review evidence, so names such as UI, transport, policy, or
telemetry do not become business concepts by folder shape alone. The analyzer
never writes these proposals to the vault.

For Markdown and reStructuredText evidence, the model-visible `headings` and
`excerpt` prefer the document-title section plus explicitly classified purpose,
architecture, and ability sections. A document with no named semantic category
uses the same bounded eligible-section fallback. The risk scan judges only the
evidence that enters that packet, so an unrelated unselected peer section cannot
downgrade the claim the agent received. When one selected document mixes current evidence with
future, negated, or deprecated prose, Atlas keeps the exact current unit in
`excerpt` and carries each removed policy unit as bounded
`reviewRequiredEvidence` with its heading, one-based line span, excerpt, and
risk flags. Those records remain model-visible counterevidence but cannot
support proposal meaning. Hostile instruction/write content still taints the
whole selected row, and a split that exceeds its bound fails closed to the
original row-wide review state. Proposal validation rejects a Definition,
Includes, Excludes, relation rationale, or `answered` competency that overlaps a
review unit unless a different current semantic source independently supports
the same claim; a partial answer may retain it only as an explicit gap.

The fixed 1,200-character excerpt budget gives every selected safe section an
initial deterministic share; short sections return unused capacity by semantic
priority and source order. The document, heading, and excerpt caps do not grow.
One exact current candidate unit plus one matching implementation witness may
support only a capability proposal below `0.8` confidence. The path is not a
second semantic authority and cannot establish a domain, ownership,
completeness, an answered competency, qualification, approval, or write.

Semantic-document source addresses preserve the exact repository directory
entry case. A unique lowercase root or workspace `readme.md` therefore remains
`readme.md` in evidence, plans, and persisted competency citations on every
filesystem. Exact matches win; ambiguous case-fold matches, missing entries,
non-files, and repository escapes stay out. Package contracts do not use the
case-fold fallback and retain their conventional exact-name requirement.

Construction qualification requires executive, employee, and agent scenarios
while preserving at least four total scenarios. `fde` remains a compatible
optional audience, but it is unusable unless purpose authority declares the
exact `audience:fde` decision, the FDE CQ owner is one of the named project
meaning owners, and a current `audience-authority:fde` witness comes from a
declared purpose source. That exact witness must also reach a supported claim
and a verified citation. Merely labeling a technical question `fde` cannot
qualify it.

Analyzer-generated review and write plans use the same canonical body bytes as
a later `get_concepts({ body: "full" })` read, including exactly one structural
leading newline. After an accepted plan is written, clients must full-read every
released concept and compare bodies byte-for-byte before source connection or
finalization; trimming the strings would hide transport drift.

| `validate_vault` | **R+** Validate every doc in the vault, return `{ scanned, problems: [{slug, issues}], summary: { problemFiles, errorFiles, warningFiles, byCode }, pathDrift, evidenceDrift }`. `evidenceDrift` dates every cited `path:` and every concept document in one Git walk and states each concept `current` / `stale` (cited code changed after the document — read it before trusting the meaning) / `missing` (cited path gone) / `unknown`; `checked: false` names why nothing was dated and never means nothing moved. The public `outputSchema` is the machine-owned canonical issue-code set and restricts both `issues[].code` and `summary.byCode`; identity errors include missing/invalid UID, invalid or non-canonical merge history, and duplicate UID claims. `pathDrift` = vault→code path drift: frontmatter `path:` / `elements:` source paths missing on disk, resolved against `repoRoot` (input param, default active resolved repository root from `connection_info`) → `{ repoRoot, checked, nodesScanned, pathsChecked, drifts: [{slug, kind, key, missingPath, suggestedPath?}], hint }`. **`checked: false` (2026-08-01) means nothing was measured** — the vault is not inside a git repository and no `repoRoot` was given, so the repository it describes is unknown and any number would be noise rather than drift; `drifts` is then empty because it was not looked at, not because it is clean; ontology-slug refs are never flagged; fix via `patch_concept` or remove the stale entry. `suggestedPath` (optional, Track A #3) appears when exactly one existing repo source file shares the missing file's basename — a likely reconcile target ("the source moved here"); ambiguous (>1) or absent matches yield no suggestion. One round-trip whole-vault health check — use for first-contact before writes, before / after a batch write, or to surface issues. Replaces the K-roundtrip pattern of `list_concepts` then per-doc `get_concept` (whose `warnings: [...]` is per-file). |
| `validate_wiki` | **R+** Judge the pages under `wiki/` against the wiki page contract (`docs/ONTOLOGY-ATLAS-SPEC.md` §11): no `kind:`, the seven required frontmatter fields, the five sections in order, a citation on every bullet under `## Facts`, and a cited path both declared in `sources:` and present in the folder. A wiki page is **not** an ontology node — it carries no `kind:` by contract, which is what keeps it out of the graph — so `validate_vault` says nothing about whether one fits its own shape, and this is that answer. Codes: `kind-present`, `missing-field:<key>`, `section-order`, `uncited-fact`, `bad-citation`, `bad-truncation-record` (the optional `sources_truncated:` key — which paths in `sources:` were read only in part — is not a list of paths, or names one the page does not cite), `citation-target-missing`, `describes-needs-approval`, plus `not-a-wiki-page` and `page-missing` for a path that names neither. The folder is judged too, over every page whatever `paths[]` narrowed the report to, and its findings land on the page they concern: `dangling-wikilink` (a `[[wiki/…]]` target not in the folder), `orphan-page` (nothing links here; a folder of one page reports nothing), `shared-source-unlinked` (another page lists a source this one lists and neither links the other; on both). Optional `paths[]` narrows the run, capped at 50 — the same ceiling `get_concepts.uids` uses, because past that asking for the whole folder is one call instead of a list somebody has to assemble. Omitted, it judges every page under `wiki/` except the shipped `_template.md`, whose own citations name a placeholder, and has no cap: the folder decides how many pages there are. Returns `{ pageCount, failingCount, pages: [{path, ok, problems: [{code, message, line?}]}] }` — the same shape `ontology-atlas wiki-validate --json` prints, so a person and an agent read one report. **side effect 0.** Call it after writing or editing a page, and before claiming a compile finished. |
| `read_source` | **R** Read the text of one raw source under `sources/`, cut into the units a wiki citation names: a DOCX by heading (`h:<slug>`; repeated normalized headings receive distinct generated anchors, while paragraphs before the first heading remain `p1`), an XLSX by sheet and row (`s<n>r<m>`), a CSV/TSV by quoted record (`r<n>` names its physical starting line), and a text or HTML file by line (`l<n>`). Each unit carries the exact anchor for `[[src:sources/<file>#<anchor>]]`, so a page cites what it quotes rather than a place it guessed. A PDF returns no text — the agent runtime reads PDFs natively, page by page, and cites `#p<n>`. Nothing is converted and kept: the file is read on request and the text returned once, with its `sha256` (the same `source_hash` a page records). `from` and `limit` page the units (default 200, max 1000); `truncated` plus `next` say where to continue; `sheet` narrows a workbook to one sheet. CSV pagination counts non-empty records, so a next-unit cursor is not a row anchor: a multiline record at `r2` can be followed by `r4`. Quoted newlines and spaces stay in the returned record; an unclosed quoted remainder is retained with a warning note, not certified as valid CSV. DOCX disambiguation reserves all natural heading names before generating suffixes; previously unique anchors remain unchanged. A note names ambiguous legacy heading addresses instead of silently assigning one occurrence to an old ambiguous address. Existing wiki citations are not rewritten, and a byte-hash match does not resolve such ambiguity. The path must sit under `sources/` and inside the folder. **side effect 0.** It exists so a Compile, Check or ask turn in the app reads a DOCX or XLSX without a shell command, which is what used to raise an execute permission card on a turn that only reads. |
| `analyze_repo_structure` | **R16** Analyze a code repository (default active resolved repository root) and propose ontology node candidates from package metadata, README evidence, and source layout. **side effect 0** — vault NOT modified. Emits schema-folder-prefixed flat slugs while source locations stay in `path`/evidence, so candidates match the starter layout and CLI `add` defaults. Semantic evidence includes `trust` and `riskFlags` for instructions, future/negated claims, and deprecated state. Root `ARCHITECTURE.md` plus classified Markdown under bounded `docs/`, `site/`, and `website/` discovery can enter the existing six-document portable packet; archive-like directories and repository-escaping symlinks stay out. A root Rust package `Cargo.toml` contributes one bounded `role:"package-contract"` row containing only allowlisted `[package]` identity/description and `[features]` names/mappings. Its separate `configurationEvidence` packet covers a root package or repo-contained literal direct workspace members: bounded feature declarations plus exact literal `cfg`/`cfg_attr` feature predicate path/line, form, polarity, source role, and raw predicate. It does not evaluate predicates, execute build scripts, expand macros, claim runtime impact, or permit a relation write. Python cold starts likewise accept `README.rst`, a non-executed root `setup.py` with only static `name` / `description` / `python_requires` literals, and top-level packages identified by `__init__.py` as implementation elements. For Python packages it exposes at most 12 implementation boundaries that participate in observed imports as element/path candidates. Direct module/package boundaries remain the base; up to two exact nested security/policy/risk endpoints may reserve slots so risk ownership is not buried in a large import payload. Unused files are not mirrored, ambiguous flat slugs fail closed, and omitted lower-ranked boundaries are reported in `skipped`. A complete proposal may additionally select at most four exact TypeScript, JavaScript, Python, or Rust file endpoints already exposed by the observed import graph when they materially improve change navigation; this never adds those files to the analyzer's automatic candidate list. Package contracts, package folders, configuration predicates, and import endpoints are evidence, never automatic domains, capabilities, or semantic relations. Repository-escaping symlinks—including package-internal symlink parents—files over 256 KiB, malformed contracts, and unsupported workspace members are skipped or reported rather than guessed. A call may include the complete project/domain/capability/element/typed-relation `proposal`; `proposalValidation` checks definitions, citations, risk controls, domain/path placement, relation endpoints/types/rationales, confidence, and five typed competency answers. `Excludes` may contain only sourced product/concept boundaries; unknown, unmeasured, or unproven evidence limits belong in `Uncertainty` or a competency gap, and `epistemic-exclusion-boundary` fails closed before such a scope claim can reach a write plan. A TS/JS/Python/Rust `depends_on` backed by an admitted boundary or selectively proposed exact endpoint must match the observed module/file import direction or validation fails. Each answer carries `answered` / `partial` / `visible-gap` status plus concept, relation, evidence, and path witnesses. Unsupported `answered` claims fail closed; every honest warning becomes an exact `requiredGapId`. The first valid proposal call returns only a deterministic non-writing `reviewPlan`, `planDigest`, `planRevision`, authoritative current `sourceDigest`, and eight-phase `constructionLifecycle`; `canWrite` stays false and `writePlan` is absent. A separately identified evaluator must execute the approved CQs, current claim/citation checks, all seven quality axes, complete source-hidden task, and cold-start or prior-CQ regression. Each qualification claim must bind to the exact plan with `proposalRefs`; the lifecycle returns `proposalCoverage` and fails closed on missing, foreign, or source-hidden-uncovered refs. After the user sees the exact plan and gaps, declared human acceptance is bound to the returned digest/revision/gap ids in a `constructionQualification:v1` packet. Only an unchanged second call whose packet is admissible returns `writeEligibility:"executable"`, `canWrite:true`, and a `writePlan` exactly equal to the reviewed rows. Declared approval is provenance, not identity authentication or a truth certificate. The detailed lifecycle packet stays in the MCP response/agent transcript; persisted truth remains the existing project competency body plus finalizer receipt, so past exact gap approval is not reconstructed after restart. Write relations only after every released concept row succeeds, then validate, compile, connect the source, and finalize project meaning. Detects FSD vs generic layout. |
| `infer_imports` | **R17** Walk TS/JS files, root and `src`/`source`-layout Python packages, deterministic Rust source, and root-module Go source → file-import plus typed package-import evidence. **side effect 0**. Structured `coverage` names Go, JavaScript, Python, Rust, and TypeScript support; C/Autotools remains fail-closed. Rust reuses the existing file/module envelope for resolvable `use`, file-backed `mod`, and exact literal path/include forms, capped at 256 KiB and 256 dependency statements per file. It rejects repository escapes and does not expand macros, evaluate `cfg`, resolve symbols, run Cargo/compiler/network code, or infer runtime, reverse, transitive, capability, or business impact. Conditional and non-literal forms remain `unsupported-static-form`; external crate names are observed candidates whose importance requires Cargo/package evidence and review. Go never fabricates an imported target file: optional `packageImportEvidence` (`goPackageImports:v1`) preserves exact importing files and repository-relative package directories with bounded receipts. `value` means “not explicit type-only syntax”, never proven runtime execution. Import/package direction is observed evidence, not a self-approving semantic dependency or runtime impact. Focus mode pages exact file edges; compact Go delivery preserves an explicit full-evidence call. Omit `reviewMode` for size-safe delivery; `{reviewMode:"next"}` returns one bounded reconciliation packet, while an intentionally large complete response requires `{reviewMode:"full",allowLargeResponse:true}`. Missing edges remain `rationale_review_required`, never `proposedAction`. A path in an unsupported language such as C or with unknown implementation remains `notJudgeableByImports`; even in supported languages, **"No import found" is not evidence of absence.** Inspect both concepts and exact evidence, explain the meaning-level dependency, and ask the user before one explicit `add_relation(..., why)`. `index_project` reuses one full receipt instead of rescanning. |
| `inspect_architecture` | **R+** Read one reviewed `architecture-profile/v1` and compare its scoped role/path/dependency rules with current supported source imports. Optional `dependency_usages` declares a non-empty subset of `value` / `type_only`; missing v1 fields preserve both, while `unknown` is never declarable. Returns `architectureBrief:v1`, role coverage, usage-qualified observed role edges and receipts, bounded violations, explicit unknowns, and the required `architectureChangePlan:v1` fields. Pattern names are declarations, not folder-name inference. Unsupported languages, incomplete scans, unknown import usage, unmapped or unruled edges, and empty roles fail closed; **side effect 0**. |
...
| The web workbench composer (the map's contextual meaning editor at `/topology/?workbench=edit`, direct save) | `human` |
...
parenthetical qualifier — e.g. `title: CLI Developer Entry (57 commands —
...
add_concept({
  ...
  kind: "domain",
  title: "Payment",                       // stays the search/matching source
  labels: { ko: "Payment", en: "Payments" } // → display_ko / display_en
})
```
...
overturned (2026-08-01, `docs/DECISIONS.md` "Slugs are flat identifiers"): node
...
✓ tools/list 40/40 (40/40 titled; 24/24 read; 16/16 write; 9/9 destructive; 3/3 idempotent; 40/40 local-only) — absorb_document · add_concept · add_concepts · add_relation · add_relations · analyze_repo_structure · compile_ontology · connect_project_source · connection_info · delete_concept · disconnect_project_source · finalize_project_meaning · find_backlinks · find_evidence · find_neighbors · find_orphans · find_path · get_concept · get_concepts · get_constellation · git_history · git_snapshot · git_status · index_project · infer_imports · inspect_architecture · list_concepts · list_constellations · list_kinds · merge_concepts · patch_concept · query_concepts · query_ontology · read_source · reclassify_concept · remove_relation · rename_concept · replace_relation · validate_vault · validate_wiki
