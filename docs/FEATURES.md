...
| **Atlas Current** (2026-09-25) | A standalone, local build of the vault and its Git history (`packages/atlas-current`): map, tiers, a time view of every concept-page commit with a forecast slice of what changes next (hits filled, misses crossed, slices brightened by surprise), route trace and a three.js studio. `audit` reruns the forecast with ties, base-rate calibration, not-yet-existing concepts, today's relations, repeats, commit order and splits removed, and the page's legend states its verdict (on this repository: not better than recency). Nothing leaves the machine; `serve` binds 127.0.0.1. | `pnpm current:build`, `pnpm current:audit`, then `node packages/atlas-current/bin/atlas-current.mjs serve`; in the plugin, `/atlas-current`. |
